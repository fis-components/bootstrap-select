...
(function (root, factory) {
   ...
   */
        ...
        var Selectpicker = function (element, options, e) {
            ...
            // If we have no title yet, try to pull it from the html title attribute (arguments[0] doesnt' pick it up as it's not a
            // data-attribute)
                ...
                this.$element.find('option').each(function (index) {
                    ...
                    if (this.parentElement.tagName === 'OPTGROUP' && $this.data('divider') !== true) {
                        ...
                        if ($this.index() === 0) {
                            ...
                            var label = this.parentElement.label, labelSubtext = typeof $this.parent().data('subtext') !== 'undefined' ? '<small class="text-muted">' + $this.parent().data('subtext') + '</small>' : '', labelIcon = $this.parent().data('icon') ? '<span class="' + that.options.iconBase + ' ' + $this.parent().data('icon') + '"></span> ' : '';
     ...
     */
            ...
            liHeight: function (refresh) {
                ...
                var liHeight = a.offsetHeight, headerHeight = header ? header.offsetHeight : 0, searchHeight = search ? search.offsetHeight : 0, actionsHeight = actions ? actions.offsetHeight : 0, doneButtonHeight = doneButton ? doneButton.offsetHeight : 0, dividerHeight = $(divider).outerHeight(true),
                    // fall back to arguments[0] if getComputedStyle is not supported
